...
import { faSpinner, faUserClock, faArrowLeft } from "@fortawesome/free-solid-svg-icons";
...
export default function AddAppointment() {
    ...
    useEffect(() => {
        (() => {
            const currentFormConfig = formElements[0]?.addAppointment;
            const formTitle = currentFormConfig?.titleForm || "Enregistrer un rendez-vous";
    ...
    const handleInputChange = (alias: string, value: any) => {
        ...
        try {
            localStorage.setItem("inputMemoryOfAddAppointmentPage", JSON.stringify(updatedInputs));
        } catch (e) {
            ...
            <div className="mx-auto max-w-5xl space-y-8">
                ...
                <div className="flex flex-col gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
                    ...
                    <div className="flex items-center gap-3 mt-2">
                        <Link
                            href="/dashboard/APPOINTMENT/list"
                        ...
                        >
                            ...
                            <span>Retour à la liste des rendez-vous</span>
                        </Link>
                    </div>
                </div>
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                                    Remplissez les informations ci-dessous pour planifier une visite à l'accueil.
                                </p>
                            </div>
                        </div>
                            ...
                            {inputList.map((e: any, index: number) => {
                                ...
                                return (
                                    <div key={index} className={`flex flex-col gap-2 ${e.textarea ? "md:col-span-2" : ""}`}>
                                        ...
                                        {/* Input Standard (text, email, date, time, tel...) */}
                                        {!e.selectedInput && !e.textarea && (
                                            <input
                                                type={e.type || "text"}
                                                value={value}
                                                maxLength={e.type === "tel" ? 10 : undefined}
                                        ...
                                        {/* Select (Choix de l'interlocuteur UserId, statut, etc.) */}
                                        {e.selectedInput && (
                                            <select
                                                value={value}
                                                onChange={(v) => {
                                                    const val = e.type === "number" ? parseInt(v.target.value, 10) : v.target.value;
                                        ...
                                        <p className={requireFields[e.alias] ? "text-xs  text-red-500" : "hidden"}>
                                            {requireFields[e.alias]}
                                        </p>
                                    </div>
                                );
                            })}
