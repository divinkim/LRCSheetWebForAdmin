  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-800 dark:bg-slate-950 dark:text-slate-100 p-4 sm:p-6 lg:p-8 transition-colors duration-200">
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        {formElements.map((element, idx) => (
          <div key={idx} className="flex flex-col gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  Renseignez les détails de la rémunération ci-dessous.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              {element.addOrUpdateSalary.navigationLinks.map((navItem, index) => {
                // Masquer selon le rôle admin si nécessaire
        ...
        {/* 📝 FORM CARD CONTAINER */}
        <div className="mx-auto max-w-4xl overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900/50 shadow-sm dark:shadow-xl transition-all duration-200">
            ...
            <div key={idx} className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 px-6 py-5">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                  {element.addOrUpdateSalary.addSalaryTitleForm}
                </h2>
              </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {formElements.map((element) =>
                element.addOrUpdateSalary.inputs.map((e) => {
                  // Règle de masquage
                  const isHidden = e.alias === "adminService" && inputs.role === null;
                  if (isHidden) return null;

                  return (
                    <div key={e.alias} className={cn("flex flex-col gap-2", e.type === "textarea" && "md:col-span-2")}>
                      <label className="text-sm font-semibold text-slate-600 dark:text-slate-300 tracking-wider">
                        {e.requireField && <span className="mr-1 text-amber-500 font-bold">*</span>}
                        {e.label}
                      </label>

                      {/* Input texte / nombre / tel / file */}
                      {!e.selectedInput && e.type !== "textarea" && (
                        <input
                          type={e.type}
                          ...
                          onChange={(v) => {
                            const field = e.alias;
                            const value = e.type === "number" ? parseInt(v.target.value) || "" : v.target.value;
                            const updated = { ...inputs, [field]: value };
                            setInputs(updated);
                            localStorage.setItem("inputMemoryOfAddSalaryPage", JSON.stringify(updated));
                          }}
                      ...
                      {!e.selectedInput && e.type === "textarea" && (
                        <textarea
                          ...
                          onChange={(v) => {
                            const field = e.alias;
                            const updated = { ...inputs, [field]: v.target.value };
                            setInputs(updated);
                            localStorage.setItem("inputMemoryOfAddSalaryPage", JSON.stringify(updated));
                          }}
