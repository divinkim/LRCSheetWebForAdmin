      ...
      <div className="mx-auto max-w-7xl space-y-8">
          ...
          <div key={idx} className="flex flex-col gap-4 border-b border-slate-200 dark:border-slate-800 pb-6">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                Renseignez les informations ci-dessous pour créer ou modifier un département.
              </p>
            </div>

            {/* Action Links Bar */}
            {/* <div className="flex flex-wrap items-center gap-3">
              {element.addOrEditDepartment.navigationLinks.map((link, index) => {
                // Masquer selon le rôle si nécessaire
        ...
        <div className="overflow-hidden rounded-2xl border border-slate-200/80 dark:border-slate-800 bg-white dark:bg-slate-900/50 shadow-sm dark:shadow-xl transition-all duration-200">
            ...
            <div key={idx} className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 px-6 py-5">
              <div className="flex items-center gap-2.5">
                ...
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                  {element.addOrEditDepartment.addDepartmentTitle}
                </h2>
              </div>
          ...
          <div className="p-6 sm:p-8">
            ...
            {/* Preview Photo Upload (Affiche l'aperçu si une image existe dans les inputs) */}
            {inputs.photo && (
              ...
              {formElements.map((element) =>
                element.addOrEditDepartment.inputs.map((e, index) => {
                  ...
                  return (
                    <div
                      key={e.alias || index}
                      className={cn("flex flex-col gap-2", e.textarea && "md:col-span-2")}
                    >
                      <label htmlFor={e.alias} className="text-sm font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">
                      ...
                      {!e.selectedInput && !e.textarea ? (
                        <input
                          ...
                          onChange={async (v) => {
                            const field = e.alias;
                            let valueToSave = v.target.value;
                            ...
                            ? dynamicArrayData
                              ...
                              ?.arrayData?.map((option: any, optIdx: number) => (
                                <option
                                  key={optIdx}
                                  value={option.value}
                                  className="text-slate-800 dark:bg-slate-900 dark:text-slate-200"
                                >
                                  {option.title}
                                </option>
                              ))
