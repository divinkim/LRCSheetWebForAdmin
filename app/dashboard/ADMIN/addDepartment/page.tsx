...
export default function AddPost() {
    const { dynamicArrayData, staticArrayData, inputs, setInputs, isLoading, handleSubmit, adminRole } = useAddDepartment();
    return (
        <main className="bg-white dark:bg-transparent">
            <div className="flex">
                <div className="mx-4 mt-6 mb-4 w-full font-semibold">
                        ...
                        formElements.map((element) => (
                            <div className="text-gray-700 w-full space-y-4 md:space-y-0 items-center">
                                <div className="flex justify-between flex-wrap">
                                    <h1 className="font-bold mb-3 text-[20px] dark:text-gray-300 text-gray-700">Ajouter un nouveau département</h1>
                                    <p className="text-blue-700 dark:text-blue-600">Dashboard/RH/Ajouter un département</p>
                                </div>
                                <hr className='bg-gray-400' />
                                <div className="flex flex-wrap py-4 lg:space-x-4 space-y-4 items-center">
                                    {
                                        element.addOrEditDepartment.navigationLinks.map((element, index) => (
                    ...
                    <div className='dark:border mt-8 w-full lg:w-3/4 mx-auto font-semibold h-auto border-gray-400 dark:border-gray-800 dark:bg-gray-900 rounded-[30px] border  dark:shadow-none p-4'>
                        {
                            formElements.map((element) => (

                                <div className="flex flex-wrap space-y-4 justify-between mb-2 items-center dark:text-gray-300 text-gray-700">
                                    <h2 className="font-bold">{element.addOrEditDepartment.addDepartmentTitle}</h2>
                        ...
                        <div className='grid grid-cols-1 mt-4 gap-x-4 md:grid-cols-2 font-semibold w-full'>
                            {
                                formElements.map((element) => (
                                    ...
                                    element.addOrEditDepartment.inputs.map((e, index) => (
                                        <div className={cn('w-full mb-4')}>
                                            ...
                                            {!e.selectedInput && !e.textarea ?
                                                <input value={inputs[e.alias] ?? ""} onChange={async (v) => {
                                                    ...
                                                    window?.localStorage.setItem("inputMemoryOfAddDepartmentPage", JSON.stringify({ ...inputs, [field]: v.target.value }));
                                                ...
                                                e.selectedInput && !e.textarea ?
                                                    <select value={inputs[e.alias] ?? ""} onChange={(v) => {
                                                        ...
                                                        localStorage.setItem("inputMemoryOfAddDepartmentPage", JSON.stringify(fieldValue))
