...
export default function UsersList() {
  ...
  const stats = useMemo(() => {
    ...
    const totalEnterprises = new Set(
      usersList.map((u) => u.Enterprise?.name).filter(Boolean)
    ).size;
  ...
  const exportToCSV = useCallback(() => {
    ...
    link.setAttribute(
      "download",
      `collaborateurs_export_${new Date().toISOString().slice(0, 10)}.csv`
    );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div className="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-extrabold text-slate-900 dark:text-slate-100 mt-1">
                {loading ? "-" : stats.totalEnterprises}
              </p>
            </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={8} className="py-12 text-center text-slate-500">
                      ...
                      />
                      <p className="font-medium">Chargement des collaborateurs...</p>
                    </td>
                  </tr>
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            onClick={() =>
                              checkAccessAndExecute(() => {
                                router.push(
                                  `/dashboard/RH/getUserProfile/${user.id}`
                                );
                              })
                            }
                          ...
                          <button
                            ...
                            onClick={() =>
                              checkAccessAndExecute(() => {
                                router.push(
                                  `/dashboard/RH/updateUser/${user.id}`
                                );
                              })
                            }
